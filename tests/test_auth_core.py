...
class AuthCoreTests(unittest.TestCase):
    ...
    def test_auth_accepts_env_values_pasted_with_names(self):
        salt = b'test-salt'
        iterations = 10000
        expected = hashlib.pbkdf2_hmac('sha256', b'correct-password', salt, iterations, dklen=32)
        password_hash = 'pbkdf2_sha256${}${}${}'.format(
        ...
        with patch.dict(os.environ, {
            ...
            'ADMIN_PASSWORD_HASH': 'ADMIN_PASSWORD_HASH={}'.format(password_hash),
            'AUTH_SECRET': 'AUTH_SECRET=test-auth-secret',
        }, clear=False):
            self.assertTrue(app.verify_password('correct-password', os.environ['ADMIN_PASSWORD_HASH']))
            cookie = app.create_session_cookie(app.auth_email(), app.auth_secret())
            cookie_value = cookie.split(';', 1)[0].split('=', 1)[1]
            user = app.session_user('{}={}'.format(app.SESSION_COOKIE, cookie_value))
            self.assertEqual(user['email'], 'admin@example.com')
